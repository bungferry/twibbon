...
<script>
    ...
    export default {
        setup() {
            ...
            // ------------------------------------
            // FUNGSI SUPABASE (SUDAH DIPERBAIKI)
            // ------------------------------------
            ...
            async function trackSupport() {
              try {
                // 🛑 PERBAIKAN: Hapus penambahan lokal. Angka akan di-update oleh Real-time.
            ...
            function incrementManualSupport() {
                ...
                // Kirim 1000 permintaan secara independen
                for (let i = 0; i < INCREMENT_AMOUNT; i++) {
                    // Panggil RPC Supabase langsung (tanpa async/await)
                ...
                .on(
                  ...
                  (payload) => {
                    if (payload.new && payload.new.count_total !== undefined) {
                      // INI SATU-SATUNYA TEMPAT YANG MENGUBAH supportCount.value
