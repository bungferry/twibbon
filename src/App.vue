<template>
  <div class="app">
      ...
      <div class="controls">
        ...
        <div class="support-count-header">
          ...
          <span v-else class="count-number">
            {{ supportCount.toLocaleString('id-ID') }} Dukungan </span>
        </div>
      </div>
    </div>
    <div class="canvas-wrap" @dragover.prevent="onDragOver" @dragleave="onDragLeave" @drop.prevent="onDrop" @click="triggerUploadIfNoImage">
      ...
      <div v-if="!imageLoaded" :class="['drop-area', { hover: isDragOver }]">
        <div class="drop-area-text">
          <i class="fas fa-image"></i> Seret & lepas gambar di sini
        </div>
    ...
    <div class="toolbar">
      <small class="info">
        <span v-if="imageLoaded"> Seret untuk memindahkan. Cubit dua jari untuk memperbesar. Scroll untuk zoom (desktop). </span>
        <span v-else> Silakan unggah gambar terbaik Anda di atas untuk mulai mengedit. </span>
      </small>
    </div>
  </div>
</template>
    ...
    setup() {
      ...
      async function fetchSupportCount() {
        try {
          const {
            data,
            error
          } = await supabase.from('metrics').select('count_total').eq('id', TWIBBON_METRIC_ID).single();
          if (error) {
      ...
      async function trackSupport() {
        try {
          ...
          const {
            error
          } = await supabase.rpc('increment_twibbon_count', {
      ...
      function subscribeToSupportChanges() {
        ...
        const supportChannel = supabase.channel('twibbon-support-channel').on('postgres_changes', {
            event: 'UPDATE',
            schema: 'public',
            table: 'metrics',
